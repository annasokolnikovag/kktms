...
procedure TfMain.ToExcelClick(Sender: TObject);
var
  ExcelApp: Variant;
  Workbook: Variant;
  Sheet: Variant;
...
begin
  ...
  try
    ...
    {копируем данные из сетки в ячейки листа}
    for i := 0 to SG.RowCount - 1 do
      for j := 0 to SG.ColCount - 1 do
        Sheet.Cells[i+1,j+1] := SG.Cells[j,i];
     //определяем размеры ячейки:
        Rect := Sheet.Cells[i+1,j+1].Cells.Borders.Item[1].LineStyle;
        Rect.Left := Sheet.Cells[i+1,j+1].Left;
        Rect.Top := Sheet.Cells[i+1,j+1].Top;
        Rect.Right := Sheet.Cells[i+1,j+1].Left + Sheet.Cells[i+1,j+1].Width;
        Rect.Bottom := Sheet.Cells[i+1,j+1].Top + Sheet.Cells[i+1,j+1].Height;
        //редактируем высоту строки в зависимости от высоты текста:
        Sheet.Rows[i+1].RowHeight := Sheet.Canvas.TextHeight(SG.Cells[j,i]) + 4;
        //редактируем ширину столбца в зависимости от ширины текста:
        Sheet.Columns[j+1].ColumnWidth := Sheet.Canvas.TextWidth(SG.Cells[j,i]) + 4;
        //записываем значение ячейки еще раз, чтобы оно автоматически подстроилось под новые размеры:
        Sheet.Cells[i+1,j+1] := SG.Cells[j,i];
      end;
    ...
    on E: Exception do
      ShowMessage('Ошибка экспорта в Excel: ' + E.Message);
  end;
end;
